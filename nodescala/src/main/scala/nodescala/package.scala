...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def always[T](value: T): Future[T] = {
      val promiseSuccess = Promise.successful(value)
      promiseSuccess.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = {
      if (f.value.isEmpty) throw new NoSuchElementException
      else Await.result(f, 0 milliseconds)
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val promiseS = Promise[S]()
      f onComplete(_ => promiseS.success(cont(f)))
      promiseS.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val promiseS = Promise[S]()
      f onComplete ((t: Try[T]) => promiseS.success(cont(t)))
      promiseS.future
    }
